			...
			<?php							
				if(mysqli_num_rows($sql2)>0){
					...
					while ($row = mysqli_fetch_array($sql2)) {
						...
						echo "<td>".$row[0]."</td>";
						echo "<td>".$row[1]."</td>";						
						echo "<td width='3%'><center><a href='".url()."model/formularios/postpersonalm.php?r=".$r[0]."' class='btn btn-primary btn-sm' target='_self'><i class='fa fa-download'></i></a></center></td>";
						echo "</tr>";
					}
					...
					*/
					...
					if($total_pag>1){		 		
				 		if($start!=1){
				 			echo "<li><a href=\"javascript:paginarPostPersonalm('".($start-1)."','".$cmt."', '".$lmd."', '".$anio."')\">&laquo; Anterior</a></li>";				 			
				 		}
				 		for ($j=$start; $j <= $end; $j++) {
				 			if($pag==$j){
				 				echo "<li class='active'><span>".$pag."</span></li>";
				 			}else{
				 				echo "<li><a href=\"javascript:paginarPostPersonalm('".$j."','".$cmt."', '".$lmd."',  '".$anio."')\">".$j."</a></li>";
				 			}		 			
				 		}
						/*
						...
						*/
				 		if($j<=$total_pag){
				 			echo "<li><a href=\"javascript:paginarPostPersonalm('".($j)."','".$cmt."', '".$lmd."',  '".$anio."')\" aria-hidden='true'>Siguiente &raquo;</a></li>";
				 		}
